...
function matchConstraint(constraint, value) {
  ...
  if(constraint.$containsTypeInsensitive) {
    var containsTypeInsensitive = constraint.$containsTypeInsensitive.toString();
    value = [].concat(value);

    var result = value.some(function(value) {
      return value.toString() === containsTypeInsensitive;
    });

    if(!result) {
      return value + " does not contains " + containsTypeInsensitive;
    }
